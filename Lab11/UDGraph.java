...
public class UDGraph {
     ...
     * "this" graph. *** DO NOT CHANGE "this" GRAPH!!! ***
     *
        ...
        for (int i = 0; i < vertices; i++) {
            for (int j = 0; j < vertices; j++) {
                for (int k = 0; k < vertices; k++) {
                    if (hasEdge(i, j) && hasEdge(j, k)) { newGraph.addEdge(i, k); }
                }
            }
        }
    ...
    public UDGraph paths(int length) {
        ...
        if(length == 2) { newGraph = length2Paths(); }
            ...
            /* Recursively calls paths. Creates a temporary graph to hold graph edges
             * from previously called path method and uses it to find the edge to the
             * next called path method until it reaches length - 1. */
            UDGraph tempGraph = paths(length - 1);
            for(int i = 0; i < vertices; i++) {
                for(int j = 0; j < vertices; j++) {
                    for(int k = 0; k < vertices; k++) {
                        if(tempGraph.hasEdge(i, j) && hasEdge(j, k)) {

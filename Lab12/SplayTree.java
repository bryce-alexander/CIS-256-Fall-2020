...
public class SplayTree implements Dictionary {
    ...
    /**
     * rotateRight() rotates a node up through its parent. It works only if "node"
    ...
    /**
     * rotateLeft() rotates a node up through its parent. It works only if "node" is
    ...
    /**
     * zig() rotates "node" up through its parent. (Note that this may entail either
     * a rotation right or a rotation left.)
     *
    ...
    /**
     * splayNode() splays "node" to the root of the tree with a sequence of zig-zag,
     * zig-zig, and zig operations.
     *
    ...
    private void splayNode(BinaryTreeNode node) {
        ...
        while (node != root) {
            ...
            // zig case
            if(grandParent == null) { zig(node); }

            // zig-zig case
            else if((parent == grandParent.leftChild) == (node == parent.leftChild)) {
                zigZig(node);
            }
     ...
     * the dictionary, including calling splayNode() on the new node.
     *

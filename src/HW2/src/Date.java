...
class Date {
    ...
    public Date(String s) {
        ...
        String marker = s.valueOf('/');
        String month = s.substring(0, s.indexOf(marker));

        s = s.substring(s.indexOf(marker) + 1, s.length());

        String day = s.substring(0, s.indexOf(marker));
        String year = s.substring(s.indexOf(marker) + 1, s.length());
    ...
    public static int daysInMonth(int month, int year) {
        ...
        return numOfDays; // replace this line with your solution
    }
    ...
    public static boolean isValidDate(int month, int day, int year) {
        ...
        if ((month > 0) && (month < 13) && (day > 0) && (day < 32)) {
            if (isLeapYear(year) && (daysInMonth(month, year) == 29)) {
                return true;
            } else if ((!isLeapYear(year)) && (daysInMonth(month, year) == 28)) {
                return true;
    ...
    public String toString() {
        return String.valueOf(month) + "/" + String.valueOf(day) + "/" + String.valueOf(year); // replace this line with
        // your solution
    }
    ...
    public int dayInYear() {
        ...
        switch (this.month) {
            case 1:
                ...
                break;
            case 2:
                julianDate = 31 + this.day;
                break;
            case 3:
                julianDate = 59 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 4:
                julianDate = 90 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 5:
                julianDate = 120 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 6:
                julianDate = 151 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 7:
                julianDate = 181 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 8:
                julianDate = 212 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 9:
                julianDate = 243 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 10:
                julianDate = 273 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 11:
                julianDate = 304 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
            case 12:
                julianDate = 334 + this.day;
                if (isLeapYear(this.year)) {
                    julianDate += 1;
                }
                break;
        ...
        return julianDate; // replace this line with your solution
    }
    ...
    public int difference(Date d) {
        ...
        int numOfLeapYears = 1;

        if (this.year == d.year) {
            differenceInDays = dayInYear() - d.dayInYear();
        }

        if (this.year > d.year) {
            if (this.year - d.year > 4) {
                numOfLeapYears += ((this.year - d.year) / 4);
                differenceInDays = (365 * (this.year - d.year)) - (d.dayInYear() - dayInYear()) + numOfLeapYears;
            } else {
                differenceInDays = (365 * (this.year - d.year)) - (d.dayInYear() - dayInYear());
            }
        }

        if (this.year < d.year) {
            if (d.year - this.year > 4) {
                numOfLeapYears += ((d.year - this.year) / 4);
                differenceInDays = (365 * (this.year - d.year)) + (dayInYear() - d.dayInYear()) - numOfLeapYears;
            } else {
                differenceInDays = (365 * (this.year - d.year)) + (dayInYear() - d.dayInYear());
            }
        }

        return differenceInDays; // replace this line with your solution
    }

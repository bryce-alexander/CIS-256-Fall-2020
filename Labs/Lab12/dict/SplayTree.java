...
public class SplayTree implements Dictionary {
   ...
   **/
  private void rotateRight(dict.BinaryTreeNode node) {
    if (node == null || node.parent == null || node.parent.leftChild != node) {
      System.out.println("Illegal call to rotateRight().  You have bug #1.");
      return;
    }

    dict.BinaryTreeNode exParent = node.parent;
    dict.BinaryTreeNode subtreeParent = exParent.parent;
   ...
   *  @param node the node to rotate up through its parent.
   **/
  private void rotateLeft(dict.BinaryTreeNode node) {
   ...
   **/
  private void zig(dict.BinaryTreeNode node) {
    if (node == null || node.parent == null) {
   ...
   **/
  private void zigZag(dict.BinaryTreeNode node) {
    if (node == node.parent.leftChild) {
      rotateRight(node);
      rotateLeft(node);
    } else {
   ...
   *  @param node the node to splay two steps up the tree.
   **/
  private void zigZig(dict.BinaryTreeNode node) {
   ...
   **/
  private void splayNode(dict.BinaryTreeNode node) {
    // When you do Part II of the lab, please replace the following faulty code
    // with your solution.
    while (node.parent != null) {
      dict.BinaryTreeNode parent = node.parent;
      dict.BinaryTreeNode grandparent = node.parent.parent;
      if (parent==root) {
        zig(node);
      } else if ((node == parent.rightChild && parent == grandparent.leftChild) || (node == parent.leftChild && parent == grandparent.rightChild)) {
        zigZag(node);
      } else {
        zigZig(node);
      }

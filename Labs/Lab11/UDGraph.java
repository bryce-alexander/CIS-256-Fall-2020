...
public class UDGraph
{
  ...
  public UDGraph length2Paths() {
    ...
    // Walk through each possible edge from v to i to find valid edges
    for (int v = 0; v<vertices; v++) {
      for (int i = 0; i < vertices; i++) {
        // If the length 1 edge from v to i exists, find all valid edges beginning at i
        if (this.hasEdge(v, i)) {
          for (int w = 0; w < vertices; w++) {
            // If an edge from i to w is valid, (v,w) is a length 2 path.
            if (this.hasEdge(i, w)) {
  ...
  public UDGraph paths(int length) {
    ...
    if (length == 0) {
      ...
      //   1. Create graph of length 2 paths
      //   2. a) For each additional length, take the starter graph (originally length 2 paths)
      //         and check for valid paths from each destination node.
      //      b) Assign valid edges with (source == previous graph's destinations) to new graph
      //         and run the loop again with your new graph serving as the starter graph
    } else {
      // Initialize a temporary graph starting with length 2 paths
      UDGraph tempGraph = length2Paths();
      // Loop for every length increment greater than 2
      while (length > 2) {
        // Initialize new graph to assign next order of edges
        UDGraph nextGraph = new UDGraph(vertices);
        for (int v = 0; v < vertices; v++) {
          for (int i = 0; i < vertices; i++) {
            // If edge exists in starter graph, check for valid edges out from that point and add them to nextGraph
            if (tempGraph.hasEdge(v, i)) {
              for (int w = 0; w < vertices; w++) {
                if (this.hasEdge(i, w)) {
                  nextGraph.addEdge(v, w);
                }
              }
            }
          }
        }
        // Reassign starter graph to nextGraph and reduce length
        tempGraph = nextGraph;
        length--;
      }
      // Assign our final graph value to the graph returned by the function
      newGraph = tempGraph;
    }
    return newGraph;
  }

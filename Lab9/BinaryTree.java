...
public class BinaryTree implements Dictionary {
    ...
    private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
        ...
        if (key.compareTo(node.entry.key()) == 0) { return node; }
            ...
            BinaryTreeNode keyFound = findHelper(key, node.leftChild);

            if (keyFound == null) {
                keyFound = findHelper(key, node.rightChild);
            }
            return keyFound;
        }
    }
        ...
        if (root == null) {
            ...
            if (node == null) { return null; }
                ...
                if(size == 1) {
                    ...
                    BinaryTreeNode minKey = findMin(root);

                    if(root.leftChild.leftChild != null) {
                        minKey.leftChild = root.leftChild;
                        root.leftChild.parent = minKey;
                        root.leftChild = null;
                        minKey.rightChild = root.rightChild;
                        root.rightChild.parent = minKey;
                ...
                if(((Comparable<Object>) node.entry.key).compareTo(node.parent.entry.key) <= 0) {

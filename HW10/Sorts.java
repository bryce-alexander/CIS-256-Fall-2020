...
public class Sorts {
    ...
    /**
     * countingSort() sorts an array of int keys according to the values of _one_ of
     * the base-16 digits of each key. "whichDigit" indicates which digit is the
     * sort key. A zero means sort on the least significant (ones) digit; a one
     * means sort on the second least significant (sixteens) digit; and so on, up to
     * a seven, which means sort on the most significant digit.
     *
     * @param key        is an array of ints. Assume no key is negative.
     * @param whichDigit is a number in 0...7 specifying which base-16 digit is the
     *                   sort key.
     * @return an array of type int, having the same length as "keys" and containing
     *         the same keys sorted according to the chosen digit.
     *
    ...
    /**
     * yell() prints an array of int keys. Each key is printed in hexadecimal (base
     * 16).
     *
    ...
    /**
     * main() creates and sorts a sample array. We recommend you add more tests of
     * your own. Your test code will not be graded.
     **/

...
public class Set {
	...
	public void insert(Comparable c) {
		// Your solution here.
		try {
			if(cardinality() == 0) { list.insertFront(c); }
			else {
				ListNode currentNode = list.back();
				
				if(c.compareTo(currentNode.item()) == 0) { return; }
				
				if(c.compareTo(currentNode.item()) > 0) { 
					currentNode.insertAfter(c); 
				} else if( (currentNode == list.front()) 
						&& (c.compareTo(currentNode.item()) < 0)) {
			...
			System.err.println("Item " + c + " is not a comparable parameter.");
		}
	}
	...
	public void union(Set s) {
		// Your solution here.
		try {
			ListNode currentNode = s.list.front();
			if(list.isEmpty()) { this.list = s.list; }
				...
				for(int i = 1; i <= list.length(); i++) {
					...
					if(i == list.length()) { insert((Comparable) currentNode.item()); }
					else {
						insert((Comparable) currentNode.item());
	...
	public void intersect(Set s) {
		// Your solution here.
		try {
			ListNode currentListNode = list.front();
			ListNode currentSNode = s.list.front();
			
			// returns nothing.
			if(list.isEmpty()) { return; }
			else if(s.list.isEmpty()) { this.list = s.list; }
			else {
				
				for(int i = 1; i <= list.length(); i++) {
					if(((Comparable) currentListNode.item()).compareTo(currentSNode.item()) < 0) {
						currentListNode = currentListNode.next();
						currentListNode.prev().remove();
					}
					else {
						currentListNode = currentListNode.next();
			...
			System.err.println("Cannot add invalid item.");
		}
	}
